...
#[cfg(target_os = "linux")]
pub async fn run_process_monitor(config: MonitorConfig, alert_tx: mpsc::UnboundedSender<AppAlert>) {
    ...
    match linux_impl::run_monitor_inner(config, alert_tx).await {
...
mod linux_impl {
    ...
    pub async fn run_monitor_inner(
        ...
        loop {
            ...
            match hdr.what {
                PROC_EVENT_EXEC => {
                    ...
                    let username =
                        pid_to_uid(pid).and_then(|uid| config.monitored_uids.get(&uid).cloned());
                    ...
                    if let Some(ref uname) = username {
                        if exe_path.is_empty() {
                            println!(
                                "netlink: detected exec event for monitored user '{}' (PID {}), but failed to read executable path (process likely exited immediately)",
                                uname, pid
                            );
                        } else {
                            println!(
                                "netlink: process launch: user='{}', pid={}, comm='{}', exe='{}'",
                                uname, pid, comm, exe_path
                            );
                        }
                    }
                    ...
                    if let Some(ref username) = username {
                        if !exe_path.is_empty() {
                            ...
                            {
                                println!(
                                    "netlink: blocking app launch via approval overlay: user='{}', pid={}, comm='{}', exe='{}'",
                                    username, pid, comm, exe_path
                                );
                                let has_overlay =
                                    apparmor::approval_policy_for_user(username).await.is_some();
                                if has_overlay
                    ...
                    if let Some(ref username) = username {
                        if !exe_path.is_empty()
                            && linux_device_policy::check_terminal_exec_block(
                                username, &exe_path, &argv,
                            )
                            .await
                        {
                            println!(
                                "netlink: blocking terminal execution: user='{}', pid={}, comm='{}', exe='{}'",
                                username, pid, comm, exe_path
                            );
                            let _ = alert_tx.send(AppAlert {
                                event_type: "app_blocked".to_string(),
                    ...
                    if let Some(decision) = if let Some(ref username) = username {
                        ...
                        if blocked {
                            println!(
                                "netlink: blocking app launch via AppArmor rule: user='{}', pid={}, comm='{}', exe='{}', rule='{}'",
                            ...
                            println!(
                                "netlink: evaluated AppArmor rule: user='{}', pid={}, comm='{}', exe='{}', disposition='{}', rule='{}'",
                                username.as_ref().unwrap(),
                                pid,
                                comm,
                                exe_path,
                ...
                PROC_EVENT_EXIT => {
                    ...
                    if let Some(process) = tracked.remove(&pid) {
                        ...
                        println!(
                            "netlink: process exit: user='{}', pid={}, comm='{}', duration={}s",
                            process.username, pid, process.comm, duration_secs
                        );
