...
impl LocalDnsController {
    ...
    pub async fn reconcile(&mut self, policies: &[DnsPolicy]) -> Result<(), String> {
        let desired_ports: HashSet<u16> =
            policies.iter().map(|policy| policy.listen_port).collect();
...
async fn handle_udp_query(
    ...
    if let Some(blocked_domain) = resolve_blocked_query(query_bytes, &context).await? {
...
mod tests {
    ...
    fn blocked_domains_match_subdomains() {
        let blocked = HashSet::from(["example.com".to_string(), "dns.google".to_string()]);
