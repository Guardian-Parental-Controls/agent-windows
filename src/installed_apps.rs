...
fn desktop_dirs_for_user(linux_username: &str) -> Vec<PathBuf> {
    ...
    if let Some(home) =
        users::get_user_by_name(linux_username).map(|user| user.home_dir().to_path_buf())
    {
...
pub fn discover_for_user(linux_username: &str) -> Vec<DiscoveredApp> {
    ...
    apps.sort_by(|left, right| {
        left.application_name
            .to_lowercase()
            .cmp(&right.application_name.to_lowercase())
    });
    apps
}
...
fn resolve_icon_png(icon_name: &str, user_home: Option<&Path>) -> Option<Vec<u8>> {
    ...
    let candidates = if trimmed.starts_with('/') {
        ...
        vec![
            ...
            PathBuf::from(format!(
                "/usr/share/icons/Adwaita/48x48/apps/{}.png",
                trimmed
            )),
        ]
    };
...
pub fn discover_for_user(_username: &str) -> Vec<DiscoveredApp> {
    Vec::new()
}
...
pub fn build_inventory_messages(
    linux_username: &str,
    apps: &[DiscoveredApp],
    report_id: &str,
) -> Vec<String> {
...
mod tests {
    ...
    fn parse_desktop_file_skips_hidden_entries() {
        let path =
            std::env::temp_dir().join(format!("timekpr-hidden-{}.desktop", std::process::id()));
        fs::write(
            &path,
            "[Desktop Entry]\nName=Hidden App\nExec=/usr/bin/hidden\nHidden=true\n",
        )
    ...
    fn parse_desktop_file_extracts_metadata() {
        let path =
            std::env::temp_dir().join(format!("timekpr-demo-{}.desktop", std::process::id()));
        fs::write(
            &path,
            "[Desktop Entry]\nName=Demo App\nExec=/usr/bin/demo\nVersion=1.2.3\nIcon=demo\n",
        )
