...
fn persist_app_policy(state: &CachedAppPolicy) -> Result<(), String> {
    let parent = PathBuf::from(APP_POLICY_PATH)
        .parent()
        .map(|p| p.to_path_buf());
...
fn persist_device_policy(state: &CachedDevicePolicy) -> Result<(), String> {
    let parent = PathBuf::from(DEVICE_POLICY_PATH)
        .parent()
        .map(|p| p.to_path_buf());
    if let Some(dir) = parent {
        fs::create_dir_all(&dir).map_err(|e| format!("failed to create policy dir: {}", e))?;
    ...
    fs::write(DEVICE_POLICY_PATH, json).map_err(|e| format!("failed to write device policy: {}", e))
}
    ...
    unsafe {
        ...
        if !bufptr.is_null() {
            windows_sys::Win32::NetworkManagement::NetManagement::NetApiBufferFree(
                bufptr as *const std::ffi::c_void,
            );
        }
    }
    ...
    match action {
        "sync_linux_device_policy" | "sync_windows_device_policy" => {
            ...
            match sync_device_policy(username, device_policy) {
                Ok(()) => (
                    true,
                    "Windows device policy synchronized".to_string(),
                    json!({}),
                ),
        ...
        "sync_apparmor_policy" | "sync_windows_app_policy" => {
            ...
            match sync_app_policy(username, approval_policy, policies) {
                Ok(()) => (
                    true,
                    "Windows application policy synchronized".to_string(),
                    json!({}),
                ),
                Err(err) => (false, err, json!({})),
            }
        ...
        "set_allowed_hours" => {
            ...
            match crate::windows_service::policy::set_allowed_hours(username, intervals) {
                Ok(()) => (
                    true,
                    "Allowed hours configured successfully".to_string(),
                    json!({}),
                ),
                Err(err) => (false, err, json!({})),
            }
        ...
        "clear_clock_tamper" => {
            ...
            (true, "Clock tamper override applied".to_string(), json!({}))
        }
        ...
        "show_overlay" => {
            ...
            (true, "Guardian overlay shown".to_string(), json!({}))
        }
        "dismiss_overlay" => {
            crate::windows_service::overlay::dismiss();
            (true, "Guardian overlay dismissed".to_string(), json!({}))
        }
        _ => (
            false,
            format!("Unknown Windows action '{}'", action),
            json!({}),
        ),
    }
}
...
pub fn discover_windows_apps(_username: &str) -> Vec<DiscoveredApp> {
    ...
    apps.sort_by(|a, b| {
        a.application_name
            .to_lowercase()
            .cmp(&b.application_name.to_lowercase())
    });
...
pub fn set_weekly_time_limits(
    _username: &str,
    _schedule: &serde_json::Map<String, serde_json::Value>,
) -> Result<(), String> {
    // Updates local weekly schedule cache
    Ok(())
}

pub fn set_allowed_hours(
    _username: &str,
    _intervals: &serde_json::Map<String, serde_json::Value>,
) -> Result<(), String> {
