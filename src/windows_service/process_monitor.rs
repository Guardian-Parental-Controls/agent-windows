...
#[cfg(target_os = "windows")]
fn resolve_active_username(rid: u32, users_map: &std::collections::HashMap<u32, String>) -> String {
    users_map
        .get(&rid)
        .cloned()
        ...
        for process in &processes {
            if crate::windows_service::bcd_integrity::should_intercept_boot_tool(
                ...
                let tree: Vec<(u32, u32, String)> = processes
                    ...
                    .collect();
                crate::windows_service::bcd_integrity::terminate_process_tree(process.pid, &tree);
            }
        }
