...
pub fn apply_screenshot_policy(
    shared: &SharedScreenshotPolicy,
...
pub fn build_screenshot_report(capture: &CapturedScreenshot) -> Result<String, String> {
    ...
    serde_json::to_string(&payload)
        .map_err(|error| format!("Failed to serialize screenshot report: {error}"))
}
...
fn encode_rgba_buffer(rgba: ImageBuffer<Rgba<u8>, Vec<u8>>) -> Option<(Vec<u8>, u32, u32)> {
    ...
    let resized = if width > MAX_CAPTURE_WIDTH {
        let new_height =
            ((height as f64) * (MAX_CAPTURE_WIDTH as f64) / (width as f64)).round() as u32;
...
fn capture_raw_image(username: &str, uid: u32) -> Option<Vec<u8>> {
    println!(
        "capture_raw_image: attempting Wayland capture for user {}",
        username
    );
    for wayland_display in ["wayland-0", "wayland-1"] {
        println!(
            "capture_raw_image: trying wayland display {}",
            wayland_display
        );
        if let Some(output) = run_as_user(
            ...
            if output.status.success() && !output.stdout.is_empty() {
                println!(
                    "capture_raw_image: wayland capture succeeded for {}",
                    wayland_display
                );
            ...
            println!(
                "capture_raw_image: run_as_user returned None for wayland display {}",
                wayland_display
            );
        }
        ...
        println!(
            "capture_raw_image: trying spectacle fallback for wayland display {}",
            wayland_display
        );
        ...
        if let Some(output) = run_as_user(
            ...
            if output.status.success() && Path::new(&temp_path).exists() {
                if let Ok(bytes) = std::fs::read(&temp_path) {
                    let _ = std::fs::remove_file(&temp_path);
                    println!(
                        "capture_raw_image: spectacle capture succeeded for display {}",
                        wayland_display
                    );
                    return Some(bytes);
                }
    ...
    println!(
        "capture_raw_image: attempting X11 capture for user {}",
        username
    );
    ...
    for display in [":0", ":1"] {
        ...
        if let Some(output) = run_as_user(username, uid, &env_vars, "scrot", &["-p", "-"]) {
            if output.status.success() && !output.stdout.is_empty() {
                println!(
                    "capture_raw_image: scrot capture succeeded for display {}",
                    display
                );
                return Some(output.stdout);
            } else {
        ...
        if let Some(output) = run_as_user(
            ...
            if output.status.success() && !output.stdout.is_empty() {
                println!(
                    "capture_raw_image: import capture succeeded for display {}",
                    display
                );
                return Some(output.stdout);
            } else {
...
fn active_window_title_kwin(username: &str, uid: u32) -> Option<String> {
    ...
    let current_exe = match std::env::current_exe() {
        ...
        Err(e) => {
            println!(
                "active_window_title_kwin: failed to get current exe path: {}",
                e
            );
    ...
    println!(
        "active_window_title_kwin: invoking active window helper for user '{}' (UID {})",
        username, uid
    );
    ...
    if let Some(output) = run_as_user(
        ...
        if output.status.success() {
            ...
            if !title.is_empty() {
                println!(
                    "active_window_title_kwin: helper succeeded, title: '{}'",
                    title
                );
                return Some(title);
            }
        ...
        println!(
            "active_window_title_kwin: failed to run helper as user {}",
            username
        );
    }
    None
...
fn active_window_title(username: &str, uid: u32) -> Option<String> {
    ...
    if let Some(title) = active_window_title_kwin(username, uid) {
        println!(
            "active_window_title: found active window via KWin: '{}'",
            title
        );
        return Some(title);
    }
    ...
    if let Some(title) = active_window_title_gnome(username, uid) {
        println!(
            "active_window_title: found active window via GNOME: '{}'",
            title
        );
        return Some(title);
    }
    ...
    for display in [":0", ":1"] {
        if let Some(output) = run_as_user(
            ...
            if output.status.success() {
                ...
                if !title.is_empty() {
                    println!(
                        "active_window_title: found active window via xdotool: '{}'",
                        title
                    );
                    return Some(title);
                }
    ...
    println!(
        "active_window_title: could not resolve active window title for {}",
        username
    );
    None
}
...
fn discover_active_usernames() -> Vec<String> {
        ...
        .output()
    {
        ...
        _ => {
            println!(
                "discover_active_usernames: loginctl execution failed or returned non-zero status"
            );
            return Vec::new();
        }
    };
    ...
    println!(
        "discover_active_usernames: raw session list:\n{}",
        session_text
    );
    for line in session_text.lines() {
        ...
        if let Ok(detail) = detail {
            ...
            println!(
                "discover_active_usernames: session {} details:\n{}",
            ...
            let is_active =
                detail_text.contains("state=active") || detail_text.contains("state=online");
            if is_graphical && is_active {
                println!(
                    "discover_active_usernames: session {} (user {}) is graphical and active",
                    session_id, username
                );
                usernames.push(username);
            } else {
                println!(
                    "discover_active_usernames: session {} (user {}) is not graphical/active: is_graphical={}, is_active={}",
                    session_id, username, is_graphical, is_active
                );
            }
        } else {
            println!(
                "discover_active_usernames: failed to show details for session {}",
                session_id
            );
        }
    }
    println!(
        "discover_active_usernames: discovered usernames: {:?}",
        usernames
    );
    usernames
}
...
fn capture_for_username(username: &str) -> Option<CapturedScreenshot> {
    println!(
        "capture_for_username: attempting capture for '{}'",
        username
    );
    let uid = match uid_for_username(username) {
        ...
        None => {
            println!(
                "capture_for_username: user '{}' UID lookup failed",
                username
            );
            return None;
        }
    ...
    let raw = match capture_raw_image(username, uid) {
        ...
        None => {
            println!(
                "capture_for_username: raw capture failed (no image bytes returned) for '{}' (UID {})",
                username, uid
            );
            return None;
        }
    ...
    let (jpeg_bytes, width, height) = match encode_image_bytes(&raw) {
        ...
        None => {
            println!(
                "capture_for_username: image encoding/resizing failed for '{}'",
                username
            );
            return None;
        }
    ...
    println!(
        "capture_for_username: screenshot successfully captured for '{}' ({}x{}, {} bytes)",
        username,
        width,
        height,
        jpeg_bytes.len()
...
mod win32 {
    use super::{CapturedScreenshot, encode_rgba_buffer};
    ...
    use windows_sys::Win32::System::RemoteDesktop::{
        WTS_CURRENT_SERVER_HANDLE, WTS_SESSION_INFOW, WTSActive, WTSConnected,
...
pub fn capture_screenshots(linux_username: Option<&str>) -> Vec<CapturedScreenshot> {
    println!(
        "capture_screenshots: requested for user: {:?}",
        linux_username
    );
    ...
    println!(
        "capture_screenshots: final target list for screenshot: {:?}",
        targets
    );
    let mut captures = Vec::new();
    ...
    println!(
        "capture_screenshots: finished capture, total screenshots: {}",
        captures.len()
    );
    captures
}
