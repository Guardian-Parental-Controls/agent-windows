...
fn reconcile_native_messaging_manifests() -> Result<(), String> {
    ...
    for dir in &manifest_dirs {
        if let Err(e) = fs::create_dir_all(dir) {
            println!(
                "Warning: Failed to create manifest directory {:?}: {}",
                dir, e
            );
            continue;
        }
        ...
        if let Err(e) = fs::write(&manifest_path, &serialized) {
            println!(
                "Warning: Failed to write Native Messaging manifest to {:?}: {}",
                manifest_path, e
            );
...
pub fn reconcile_extension_policy(
    ...
    if let Some(cp) = chrome_policy {
        if let Some(obj) = policy_json.as_object_mut() {
            ...
            if cp.safe_browsing_enforced {
                ...
                obj.insert(
                    "ForceGoogleSafeSearch".to_string(),
            ...
            obj.insert(
                "ForceYouTubeRestrict".to_string(),
                serde_json::json!(cp.youtube_restrict),
            );
            ...
            if cp.block_other_extensions {
                ...
                obj.insert(
                    "ExtensionInstallBlocklist".to_string(),
                    serde_json::json!(["*"]),
                );
                obj.insert(
            ...
            if cp.block_genai_features {
                ...
                if let Some(forcelist) = obj
                    .get_mut("ExtensionInstallForcelist")
                    .and_then(|v| v.as_array_mut())
                {
                    let genai_entry = serde_json::json!(
                        "neibhohkbmfjninidnaoacabkjonbahn;https://clients2.google.com/service/update2/crx"
                    );
    ...
    for dir in &policy_dirs {
        ...
        if let Err(e) = fs::write(&policy_path, &serialized) {
            last_err = Some(format!(
                "Failed to write Chrome policy to {:?}: {}",
                policy_path, e
            ));
            continue;
        }
...
fn reconcile_native_messaging_manifests() -> Result<(), String> {
    ...
    fs::write(&manifest_path, serialized).map_err(|e| {
        format!(
            "Failed to write Native Messaging manifest on Windows: {}",
            e
        )
    })?;
...
pub fn reconcile_extension_policy(
    ...
    let Some(_token) = agent_token else {
        ...
        unsafe {
            ...
            let ext_subkey = format!(
                r"SOFTWARE\Policies\Google\Chrome\3rdparty\extensions\{}",
                EXTENSION_ID
            );
    ...
    unsafe {
        ...
        if let Ok(settings_str) = serde_json::to_string(&settings_json) {
            ...
            let _ = set_registry_string(
                hkey,
                chrome_settings_subkey,
                "ExtensionSettings",
                &settings_str,
            );
            let _ = set_registry_string(
                hkey,
                edge_settings_subkey,
                "ExtensionSettings",
                &settings_str,
            );
        }
...
unsafe fn set_registry_string(
    ...
    use windows_sys::Win32::System::Registry::{
        KEY_WRITE, REG_OPTION_NON_VOLATILE, REG_SZ, RegCloseKey, RegCreateKeyExW, RegSetValueExW,
    };
...
unsafe fn set_registry_dword(
    ...
    use windows_sys::Win32::System::Registry::{
        KEY_WRITE, REG_DWORD, REG_OPTION_NON_VOLATILE, RegCloseKey, RegCreateKeyExW, RegSetValueExW,
    };
...
unsafe fn delete_registry_value(hkey: isize, subkey: &str, name: &str) -> Result<(), String> {
    use windows_sys::Win32::System::Registry::{
        KEY_WRITE, RegCloseKey, RegDeleteValueW, RegOpenKeyExW,
    };
    ...
    let mut hkey_result = 0;
    let status = RegOpenKeyExW(hkey, subkey_w.as_ptr(), 0, KEY_WRITE, &mut hkey_result);
...
#[cfg(target_os = "windows")]
unsafe fn write_extension_forcelist(
    hkey: isize,
    extension_id: &str,
    update_url: &str,
) -> Result<(), String> {
    ...
    if status != 0 {
        return Err(format!(
            "Failed to open/create ExtensionInstallForcelist: {}",
            status
        ));
    }
...
unsafe fn remove_extension_from_forcelist(hkey: isize, extension_id: &str) -> Result<(), String> {
    use windows_sys::Win32::System::Registry::{
        KEY_READ, KEY_WRITE, REG_SZ, RegCloseKey, RegDeleteValueW, RegEnumValueW, RegOpenKeyExW,
    };
