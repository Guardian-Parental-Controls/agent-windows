...
impl DomainPolicyRuntime {
    ...
    async fn ensure_restored(&mut self) -> Result<(), String> {
        ...
        if self.state_path.exists() {
            ...
            self.apply_state(&PersistedDomainPolicyState::default())
                .await?;
        }
    ...
    fn state_summary(&self) -> DomainPolicyStateSummary {
        DomainPolicyStateSummary {
            source_revisions: self
                ...
                .sources
                .iter()
                .map(|(source_id, source_state)| (source_id.clone(), source_state.revision.clone()))
                .collect(),
        }
    }
...
fn parse_state_value(value: serde_json::Value) -> Result<PersistedDomainPolicyState, String> {
    let Some(object) = value.as_object() else {
        return Err(
            "failed to parse domain policy state: root value must be an object".to_string(),
        );
    };
    ...
    let source_object = sources_value.as_object().ok_or_else(|| {
        "failed to parse domain policy state: sources must be an object".to_string()
    })?;
    ...
    for (source_id, source_value) in source_object {
        ...
        let source_state = if source_value.is_array() {
            ...
            let mut parsed_source: PersistedSourceState =
...
mod tests {
    use super::{
        DeviceDomainPolicyPayload, IncomingUidPolicy, PendingDomainPolicySync, PendingSourceUpdate,
        PersistedDomainPolicyState, PersistedSourceState, PersistedUidPolicy,
        build_next_state_from_pending, parse_state_value, resolve_full_sync_state,
        resolve_uid_policies,
    };
    use std::collections::{HashMap, HashSet};
