...
use super::super::baseline::{
    ADMIN_AUTH_ROLES, SECURE_BOOT_ALIASES, USB_BOOT_ALIASES, find_attribute_name,
};
use super::super::compliance::{HardwareReceipt, evaluate_secure_boot, evaluate_usb_boot};
...
pub fn audit_sysfs_interface(iface: &FirmwareInterface) -> Result<HardwareReceipt, String> {
    ...
    let mut receipt =
        HardwareReceipt::new("linux", &iface.oem, None, Some(iface.interface.clone()));
        ...
        if let Some(mechanism) = auth_mechanism(&iface.root, &role) {
            if mechanism.eq_ignore_ascii_case("certificate") {
                receipt.supervisor_password.error = Some(
                    "Certificate-based BIOS authentication is not supported by Guardian"
                        .to_string(),
                );
            }
        }
...
pub fn apply_sysfs_interface(
    ...
    if password_already_set && !force_reset_password {
        ...
        receipt.supervisor_password.applied = false;
        receipt.supervisor_password.error = Some(
            "Cannot reset an existing BIOS password without knowing the current password"
                .to_string(),
        );
    } else {
